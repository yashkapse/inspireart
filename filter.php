<?php 
    ...
    <div class="container">
        ...
        ?> 
            <h2 style="margin:10px 8px;" class="row fw-bolder">Search Results of "<?php echo $catname ?>"</h2>
        <?php } ?>
        <?php 
        $conn = mysqli_connect("localhost", "root", "", "inspireart") or die("Databse is not connect");
        $query = "SELECT * FROM gig g INNER JOIN category c ON g.category = c.category_id 
        WHERE g.art_name LIKE '%$catname%' OR c.category_name LIKE '%$catname%'";
